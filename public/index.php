...
require_once __DIR__ . '/../app/services/DateRangeResolver.php';
...
require_once __DIR__ . '/../app/services/AIWorkflowService.php';
require_once __DIR__ . '/../app/services/AgentActionService.php';
require_once __DIR__ . '/../app/services/ReportComparisonService.php';
require_once __DIR__ . '/../app/services/SimplePdf.php';
...
if (str_starts_with($uri, '/api/v1/')) {
    ...
    set_exception_handler(function (\Throwable $e) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'An internal error occurred. Please try again or contact the administrator.'
        ]);
        exit;
    });
    ...
    $authExempt = ['/api/v1/auth/login', '/api/v1/auth/forgot-password', '/api/v1/auth/reset-password', '/api/v1/sms/webhook'];

    if (!Auth::check() && !in_array($uri, $authExempt, true)) {
        Response::json(['success' => false, 'message' => 'Unauthenticated'], 401);
    }

    $isSmsWebhookRoute = $method === 'POST' && $uri === '/api/v1/sms/webhook';

    // CSRF validation for state-changing API requests
    if (in_array($method, ['POST', 'PUT', 'DELETE'], true) && !$isSmsWebhookRoute) {
    ...
    // Final-year project scope guard: departments, procurement and external/Bezaleli integration are not implemented.
    // Events/Ibada are now part of the approved workflow because attendance is recorded against a registered event.
    $outOfScopePrefixes = ['/api/v1/departments', '/api/v1/procurement', '/api/v1/bezaleli', '/api/v1/integration'];
    foreach ($outOfScopePrefixes as $prefix) {
        if (str_starts_with($uri, $prefix)) {
            Response::json(['success' => false, 'message' => 'This feature is outside the approved final-year project scope.'], 410);
        }
    }
    ...
    match (true) {
        ...
        $method === 'DELETE' && preg_match('#^/api/v1/members/(\d+)$#', $uri, $m) === 1
            => $apiController->deleteMember((int) $m[1]),

        $method === 'GET' && $uri === '/api/v1/events'
            => $apiController->listSimpleEvents(),

        $method === 'POST' && $uri === '/api/v1/events'
            => $apiController->createSimpleEvent(json_decode((string) file_get_contents('php://input'), true) ?: $_POST),

        $method === 'GET' && preg_match('#^/api/v1/events/(\d+)$#', $uri, $m) === 1
            => $apiController->getSimpleEvent((int) $m[1]),

        $method === 'PUT' && preg_match('#^/api/v1/events/(\d+)$#', $uri, $m) === 1
            => $apiController->updateSimpleEvent((int) $m[1], json_decode((string) file_get_contents('php://input'), true) ?: []),

        $method === 'DELETE' && preg_match('#^/api/v1/events/(\d+)$#', $uri, $m) === 1
            => $apiController->deleteSimpleEvent((int) $m[1]),
        ...
        $method === 'PUT' && preg_match('#^/api/v1/attendance/snapshots/(\d+)$#', $uri, $m) === 1
            => $apiController->updateAttendanceSnapshot((int) $m[1], json_decode((string) file_get_contents('php://input'), true) ?: []),

        $method === 'DELETE' && preg_match('#^/api/v1/attendance/snapshots/(\d+)$#', $uri, $m) === 1
            => $apiController->deleteAttendanceSnapshot((int) $m[1]),
        ...
        $method === 'DELETE' && preg_match('#^/api/v1/guests/(\d+)$#', $uri, $m) === 1
            => $apiController->deleteGuest((int) $m[1]),

        $method === 'GET' && $uri === '/api/v1/guests/follow-up'
            => $apiController->guestFollowUpQueue(),
        ...
        $method === 'DELETE' && preg_match('#^/api/v1/assets/(\d+)$#', $uri, $m) === 1
            => $apiController->deleteAsset((int) $m[1]),
        ...
        $method === 'PUT' && preg_match('#^/api/v1/finance/entries/(\d+)$#', $uri, $m) === 1
            => $apiController->updateFinanceEntry((int) $m[1], json_decode((string) file_get_contents('php://input'), true) ?: []),

        $method === 'DELETE' && preg_match('#^/api/v1/finance/entries/(\d+)$#', $uri, $m) === 1
            => $apiController->deleteFinanceEntry((int) $m[1]),
        ...
        $method === 'GET' && $uri === '/api/v1/settings/account'
            => $settingsController->account(),

        $method === 'PUT' && $uri === '/api/v1/settings/account'
            => $settingsController->updateAccount(json_decode((string) file_get_contents('php://input'), true) ?: []),

        $method === 'POST' && $uri === '/api/v1/settings/account/password'
            => $settingsController->changePassword(json_decode((string) file_get_contents('php://input'), true) ?: []),

        $method === 'GET' && $uri === '/api/v1/settings/preferences'
            => $settingsController->preferences(),

        $method === 'PUT' && $uri === '/api/v1/settings/preferences'
            => $settingsController->updatePreferences(json_decode((string) file_get_contents('php://input'), true) ?: []),

        $method === 'GET' && $uri === '/api/v1/settings/audit-logs'
            => $settingsController->auditLogs(),

        /* ── Communication / Messaging ── */
        $method === 'POST' && $uri === '/api/v1/sms/webhook'
            => $apiController->smsWebhook(json_decode((string) file_get_contents('php://input'), true) ?: $_POST),
        ...
        $method === 'GET' && $uri === '/api/v1/reports/comparison'
            => $reportController->comparison(),

        $method === 'GET' && $uri === '/api/v1/reports/download/pdf'
        ...
        $method === 'POST' && $uri === '/api/v1/ai/confirm'
            => $aiController->confirm(json_decode((string) file_get_contents('php://input'), true) ?: $_POST),

        $method === 'POST' && $uri === '/api/v1/ai/cancel'
            => $aiController->cancel(),
...
// GET: module pages
$webModules = ['members', 'guests', 'events', 'attendance', 'finance', 'assets', 'communication', 'reports', 'settings', 'sadaka'];
$trimmed = ltrim($uri, '/');
