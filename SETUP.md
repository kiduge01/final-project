...
### 7. Configure Email Messaging
Copy `.env.example` to `.env` and set `MAIL_FROM_ADDRESS` to a valid address on your domain. Email messages use PHP's `mail()` transport, so the server must also have a working mail transport configured.
