...
<div class="mb-6">
    ...
    <p class="text-mist-600 text-sm mt-0.5">Send SMS or email messages to members and guests, and track delivery.</p>
</div>
        ...
        <form id="compose-form" class="space-y-5">
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="recipient_type" value="guests" class="w-4 h-4 accent-royal-600">
                        <span class="text-sm font-medium text-gray-700">All Guests</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
            ...
            <!-- Delivery channel -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Delivery Channel <span class="text-red-500">*</span></label>
                <div class="flex flex-wrap gap-3">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="channel" value="sms" checked class="w-4 h-4 accent-royal-600">
                        <span class="text-sm font-medium text-gray-700">SMS</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="channel" value="email" class="w-4 h-4 accent-royal-600">
                        <span class="text-sm font-medium text-gray-700">Email</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="channel" value="both" class="w-4 h-4 accent-royal-600">
            ...
            <div id="subject-field" class="hidden">
                ...
                <input id="msg-subject" type="text" maxlength="200" placeholder="Message subject"
                    class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-royal-400">
            </div>
            ...
            <div id="member-selector" class="hidden">
                ...
                <input id="member-search" type="text" placeholder="Search by name, phone, or email" class="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm mb-2 focus:ring-2 focus:ring-royal-400">
            ...
            <div>
                ...
                <div class="flex justify-between mt-1">
                    <p id="message-help" class="text-xs text-mist-500">SMS messages &gt; 160 chars may be split into multiple parts</p>
...
function renderMemberList(members) {
    ...
    if (!members.length) { container.innerHTML = '<p class="text-sm text-gray-400">No members with contact details found.</p>'; return; }
    container.innerHTML = members.map(m =>
        `<label class="flex items-center gap-2 cursor-pointer p-1.5 hover:bg-gray-50 rounded-lg member-row" data-search="${esc((m.first_name + ' ' + m.last_name + ' ' + (m.phone || '') + ' ' + (m.email || '')).toLowerCase())}">
            ...
            <span class="text-xs text-mist-400 ml-auto">${esc(m.phone || m.email || '')}</span>
        </label>`
    ).join('');
}
...
function updateRecipientPreview() {
    ...
    if (type === 'all') {
        ...
        el.textContent = 'Will send to all guests with phone numbers';
    } else {
        const count = document.querySelectorAll('.mbr-check:checked').length;
...
document.getElementById('compose-form').addEventListener('submit', async function(e) {
    ...
    if ((channel === 'email' || channel === 'both') && !subject) { showFeedback('error', 'Please enter an email subject'); return; }
    ...
    if (!confirm(type === 'all' ? 'Send this message to all members?' : (type === 'guests' ? 'Send this message to all guests?' : 'Send this message to the selected members?'))) return;
    ...
    try {
        const res = await fetch(CAPI + '/messages/send', {
            ...
            body: JSON.stringify({ message, subject, channel, recipient_type: type, recipient_ids: recipientIds })
        });
        const data = await res.json();
...
document.querySelectorAll('input[name="channel"]').forEach(radio => {
    radio.addEventListener('change', function() {
        const emailEnabled = this.value === 'email' || this.value === 'both';
        document.getElementById('subject-field').classList.toggle('hidden', !emailEnabled);
        document.getElementById('message-help').textContent = emailEnabled
            ? 'Email messages are sent to recipients with a valid email address'
            : 'SMS messages > 160 chars may be split into multiple parts';
    });
});
...
async function loadHistory(page = 1) {
    ...
    try {
        ...
        const typeLabels = { all: 'All Members', guests: 'All Guests', members: 'Selected Members' };
        const statusClasses = {
    ...
    try {
        ...
        let html = `
            ...
            <div class="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><span class="text-gray-500">Channel:</span> <span class="font-medium">${esc(m.channel || 'sms')}</span></div>
                ${m.subject ? `<div><span class="text-gray-500">Subject:</span> <span class="font-medium">${esc(m.subject)}</span></div>` : ''}
            </div>`;
...
if (taskParams.get('action') === 'compose') {
    const channel = taskParams.get('channel');
    ...
    setTimeout(() => document.getElementById('compose-form')?.scrollIntoView({behavior:'smooth',block:'start'}), 100);
}
loadMemberOptions();
loadKPIs();
updateRecipientPreview();
