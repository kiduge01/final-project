...
<div id="guest-register-panel" class="hidden bg-white rounded-2xl border border-mist-200 shadow-sm p-5 mb-6">
    <div class="flex items-center justify-between gap-3 mb-4">
        <div>
            <h3 class="text-lg font-heading font-semibold text-royal-900">Register New Guest</h3>
            <p class="text-sm text-mist-600">Capture a guest's contact details and service visit.</p>
        </div>
        <button type="button" id="btn-close-guest-register" class="text-sm text-mist-500 hover:text-mist-700">Close</button>
    </div>

    <form id="guest-register-form" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        <div>
            <label class="block text-xs font-semibold text-mist-600 mb-1">First Name <span class="text-red-500">*</span></label>
            <input name="first_name" required class="w-full rounded-xl border border-mist-200 px-3 py-2.5 text-sm" placeholder="First name">
        </div>
        <div>
            <label class="block text-xs font-semibold text-mist-600 mb-1">Last Name <span class="text-red-500">*</span></label>
            <input name="last_name" required class="w-full rounded-xl border border-mist-200 px-3 py-2.5 text-sm" placeholder="Last name">
        </div>
        <div>
            <label class="block text-xs font-semibold text-mist-600 mb-1">Phone</label>
            <input name="phone" class="w-full rounded-xl border border-mist-200 px-3 py-2.5 text-sm" placeholder="+255 ...">
        </div>
        <div>
            <label class="block text-xs font-semibold text-mist-600 mb-1">Email</label>
            <input type="email" name="email" class="w-full rounded-xl border border-mist-200 px-3 py-2.5 text-sm" placeholder="email@example.com">
        </div>
        <div>
            <label class="block text-xs font-semibold text-mist-600 mb-1">Location</label>
            <input name="location" class="w-full rounded-xl border border-mist-200 px-3 py-2.5 text-sm" placeholder="Area or location">
        </div>
        <div>
            <label class="block text-xs font-semibold text-mist-600 mb-1">Service Date <span class="text-red-500">*</span></label>
            <input type="date" name="service_date" required class="w-full rounded-xl border border-mist-200 px-3 py-2.5 text-sm">
        </div>
        <div>
            <label class="block text-xs font-semibold text-mist-600 mb-1">Visit Type</label>
            <select name="visit_type" class="w-full rounded-xl border border-mist-200 px-3 py-2.5 text-sm">
        ...
        <div>
            ...
            <select name="status" class="w-full rounded-xl border border-mist-200 px-3 py-2.5 text-sm">
        ...
        <div class="md:col-span-2 lg:col-span-3">
            ...
            <textarea name="notes" rows="2" class="w-full rounded-xl border border-mist-200 px-3 py-2.5 text-sm" placeholder="Optional notes or follow-up details"></textarea>
        </div>
        <div class="md:col-span-2 lg:col-span-3 flex justify-end gap-3 pt-2">
            <button type="button" id="btn-cancel-guest-register" class="px-4 py-2.5 rounded-xl bg-mist-100 text-mist-700 hover:bg-mist-200 text-sm font-medium">Cancel</button>
            <button type="submit" class="px-5 py-2.5 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-semibold">Save Guest</button>
        </div>
    </form>
    <div id="guest-register-feedback" class="hidden mt-3 rounded-xl px-3 py-2 text-sm"></div>
</div>
...
function renderGuests(list) {
    ...
    tbody.innerHTML = list.map(r => `
        <tr class="hover:bg-mist-50/60">
            ...
            <td class="px-4 py-3 text-right">
                <div class="flex justify-end gap-2"><button class="text-xs text-emerald-700 font-semibold view-guest-btn" data-id="${r.id}">View</button><button class="text-xs text-royal-700 font-semibold edit-guest-btn" data-id="${r.id}">Update</button><button class="text-xs text-red-600 font-semibold delete-guest-btn" data-id="${r.id}">Delete</button></div>
            </td>
        </tr>
    `).join('');
...
const guestRegisterPanel = document.getElementById('guest-register-panel');
const guestRegisterForm = document.getElementById('guest-register-form');
const guestRegisterFeedback = document.getElementById('guest-register-feedback');

function editGuest(id) {
    const g=allGuests.find(x=>Number(x.id)===Number(id)); if(!g)return;
    editingGuestId=id; guestRegisterPanel.classList.remove('hidden');
    ['first_name','last_name','phone','email','location','age_group','visit_type','service_date','follow_up_date','status','notes'].forEach(n=>{const el=guestRegisterForm.querySelector(`[name="${n}"]`);if(el)el.value=(g[n]??'');});
    guestRegisterPanel.scrollIntoView({behavior:'smooth',block:'start'});
}

function setGuestRegisterFeedback(message, isError = false) {
    guestRegisterFeedback.textContent = message;
    guestRegisterFeedback.className = 'mt-3 rounded-xl px-3 py-2 text-sm ' + (isError ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200');
...
document.getElementById('btn-close-guest-register').addEventListener('click', () => {
    guestRegisterPanel.classList.add('hidden');
    guestRegisterForm.reset(); editingGuestId=null;
    guestRegisterFeedback.classList.add('hidden');
});

document.getElementById('btn-cancel-guest-register').addEventListener('click', () => {
    guestRegisterPanel.classList.add('hidden');
    guestRegisterForm.reset(); editingGuestId=null;
    guestRegisterFeedback.classList.add('hidden');
});

guestRegisterForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    ...
    try {
        const res = await fetch(editingGuestId ? MB + '/api/v1/guests/' + editingGuestId : MB + '/api/v1/attendance/register-guest', {
            method: editingGuestId ? 'PUT' : 'POST',
        ...
        if (!res.ok || !data.success) {
            setGuestRegisterFeedback(data.message || 'Failed to register guest.', true);
            return;
        }
